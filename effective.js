...
function getNewUser(objUserData) {
	...
	*/
	...
	const userDataCard = document.querySelectorAll('#user-card > div:last-child')[0];
    ...
    <div id="element-${objUserData["id"]}" class="row justify-content-md-center card-user">
      ...
      <div class="col-md-auto align-buttons">
        ...
        <a href="#" id="delete-${objUserData["id"]}" class="btn btn-primary"><i class="fas fa-user-slash"></i> Delete</a>
      </div>
    </div>
		<div></div>
    `;
    // BIND - eliminar la card de un usuario
    const clickDeleteUserButton = document.querySelectorAll('#delete-'+objUserData["id"])[0];
    clickDeleteUserButton.addEventListener('click', function(evento) {
			document.querySelectorAll('#element-'+objUserData["id"])[0].innerHTML = "";
			deleteUserData(objUserData["id"]);
		});
}
